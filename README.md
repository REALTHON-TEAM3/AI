...
| 기능                                | 설명                                                                                           |
|-------------------------------------|------------------------------------------------------------------------------------------------|
| **🗣️ Real-time Voice Interaction** | 텍스트 입력 없이 **목소리만으로** AI와 자연스럽게 대화할 수 있습니다.                           |
| **📝 Smart Recipe Analysis**       | 인터넷 레시피 URL 또는 텍스트를 넣으면 AI가 **단계별로 구조화된 레시피**로 자동 변환합니다.    |
| **⏰ Context-Aware Timer** (1)      | 요리 중 “면 삶는 시간 재줘” 같은 지시를 하면 **자동으로 타이머를 실행**합니다.                 |
| **⏰ Context-Aware Timer** (2)      | ‘~분 동안’, ‘~분간’, ‘~분 볶기’ 등 시간이 포함된 단계에서만 **사용자에게 먼저 타이머 시작 여부를 확인**합니다. |
| **⚡ Ultra Low Latency**           | WebSocket + OpenAI Realtime API 기반으로 **사람과 대화하듯 빠른 반응 속도**를 제공합니다.      |
